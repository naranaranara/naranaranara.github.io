...
### 경계조건 종류
  ...
  2. Neuman B.C. (미분방정식)  
  $\sigma(L) = t$  
  From constitutive relation $ \Rightarrow (E\,\underbrace{u_{,x}}_{\text{도함수}})\big|_{x=L}=t
 $
  <br>
...
given $u_o$ and $u_g$ as $\overline{j}$, $f(x)$  
 ...
 the constitutive relation $\underbrace{j}_{\text{heat flux}}=-\kappa u_{,x}$  
 such that $\underbrace{-\frac{dj}{dx}}_{\text{divergence of the heat flux}}=f(x)$ in $(0,L)$  
...
만약에 경계조건이 $x=L$에서 Neuman B.C.라면 처음에 $u_{,z}$ 먼저 계산하고 $Eu_{,x}$ 경계조건을 적용한다.  
  ...
  $\Rightarrow$ $\int_0^L w_{,x}\sigma \underbrace{A \, dx}_{\text{volume element}} = \int_0^L w fA \, dx +w(L)\underbrace{tA}_{\text{boundary force}}$
...
$-\int_0^L w_{,x}\sigma A \, dx + w\sigma A|_{0}^{L} + \int_0^L wAf \, dx =0$ $\rightarrow$ 적분법  
...
2. Weak form을 Strong form으로  
Find $u \in S =\{u| u(0)=u_o\}$  
such that $\forall w \in V =\{w|w(0)=0\}$  
$\int_0^L w_{,x}\sigma A \, dx = \int_0^L wAf \, dx +w(L)tA $ $\rightarrow$ weak form  
...
Return to $\int_0^L w(-\sigma_{,x}-f)A \, dx +w(L)A(\sigma(L)-t)=0$  
...
인라인: \( \underbrace{u_{,x}}_{\text{도함수}} \)
블록:
$$
\underbrace{\frac{d\sigma}{dx}}_{\text{미분}} + f = 0
$$
