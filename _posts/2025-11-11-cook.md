...
<img width="300" height="274" alt="image" src="https://github.com/user-attachments/assets/2331e098-2872-4a02-85ed-99ec25285422" />{: .centered }   
...
<br>  
근데 이 상태로 코드 실행하면 오류뜬다.  
왜냐면 람다값을 구할 때 분모가 0이되면서 K가 무한대로 발산하기 때문이다.  
따라서 $\nu=0.499$로 계산을 한다.  
mesh의 수를 바꿔가면서 값을 기록하게 되면  
<img width="330" height="360" alt="image" src="https://github.com/user-attachments/assets/a44b5d9f-ca87-4ba6-b223-247ad598432c" />{: .centered }   

점점 차이가 줄어들면서 수렴하는 것을 볼 수 있다.  

<img width="480" height="289" alt="IMG_0868" src="https://github.com/user-attachments/assets/3bcc509f-9889-4ef8-a3db-d9c9619e3c42" />{: .centered }   

## 고찰  
1\. $\nu=0.5$이면 비압축성이라고 한다. 그 이유는 작은 변형에서 부피변화는 $\epsilon_{vol}= \epsilon_x+\epsilon_y+\epsilon_z$  
$\epsilon_x=\epsilon$이라 하면 $\epsilon_y= -\nu*\epsilon, \epsilon_z= -\nu \epsilon$ 으로 나타낼 수 있다.  
$ \therefore \epsilon_{vol}= \epsilon (1-2nu)$ 이다. 이때 $\nu=0.5$이면 부피변화는 0이 된다.  
<br>  
2\. 물리세계와 FEM 차이  
1\) 고무재료($\nu=0.5$)는 부피는 잘 안변하지면 모양은 잘 바뀐다. 하지만 FEM에서는 변위 $u$를 몇개의 노드와 shape function으로 표현한다. 선형탄성에서는 비압축성에 가까워지면 체적변형에 대한 저항이 커진다. 즉 K가 무한대로 간다.  

2\) FEM에서 각 요소들이 부피변화가 없는 것을 잘 표현하지 못한다. $\epsilon_{vol}= \epsilon_x+\epsilon_y+\epsilon_z$ 를 요소 안 곳곳에서 만족해야한다. 하지만 만약에 자유도가 부족하면 제대로 표현할 수 없다. 따라서 변형량 자체를 0으로 보낸다. 따라서 원래 고무같은 재료들은 말랑해서 잘 움직이는데 FEM으로 해석하게 되면 안 움직이게 된다. 
