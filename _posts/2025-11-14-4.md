...
$\mathbb{I}_{ijke} \theta_{ke} = \frac{1}{2}(\theta_{ij} + \theta_{ji})$ 크로네카 델타 함수는 인덱스가 같으면 1이므로 $kl$은 1이 되어서 없어지고 $ij$만 남게 된다.  
...
given $u_i^g, \bar{t}_i, f_i$, the constitutive relation $\bar{\sigma}_{ij} = \mathbb{C}_{ijkl} \varepsilon_{kl}$  
and the kinematic relation $\varepsilon_{kl} = \frac{1}{2} \left( \frac{\partial u_k}{\partial x_l} + \frac{\partial u_l}{\partial x_k} \right)$  
...
$W_i \sigma_{ij,j} = (W_i \sigma_{ij})_{,j} - W_{i,j} \sigma_{ij}$ 이 식을 $(w_i \sigma_{ij})_{,j}$ 여기에 넣어서 식정리 한거임  

Divergence rule을 사용하면
$\int_{\partial\Omega} w_i \sigma_{ij} n_j \,dS - \int_{\Omega} w_{i,j} \sigma_{ij} \,dV + \int_{\Omega} w_i f_i \,dV = 0$
첫번째 적분을 우변으로 옮기고 노이마 경계조건을 사용하면  
