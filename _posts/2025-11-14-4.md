...
<img width="417" height="224" alt="image" src="https://github.com/user-attachments/assets/497c812d-aa5e-43f1-9fc3-12801d7b77ba" />{: .centered }  

! dof는 절점(node)가 가질 수 있는 독립적인 변수의 갯수.
! $n_{sd}$ 는 공간 차원의 수; 3차원이면 $n_{sd}=3$  

$\Omega^e$ 의 자유도 갯수는 절점의 개수 곱하기 절점 당 자유도의 수 이다.  

$$
\underline{u}_e^h = \sum_A N^A \underline{d}_e^A, \quad \underline{u}_e^h, \underline{d}_e^A \in \mathbb{R}^3
$$

Vector dofs at each node: $w_{ie}^h = \sum_A N^A c_{ie}^A; \quad i = 1, \cdots, n_{sd}$  

<img width="500" height="178" alt="KakaoTalk_20251115_150752188" src="https://github.com/user-attachments/assets/38e2fc6e-4646-440d-8d9a-984aa957a0a5" />{: .centered }  

$N^A(\xi_1, \xi_2, \xi_3) = \tilde{N}^{\bar{A}}(\xi_1) \tilde{N}^{\bar{B}}(\xi_2) \tilde{N}^{\bar{C}}(\xi_3)$  

! 틸다: parent 공간에서 정의된 함수  
! 바: 1차원 공간에서 절점 번호 1 or 2  

앞에서 했던것과 동일하게 매핑하면 된다.  
$x_{ie}(\xi) = \sum_A N^A(\xi) x_{ie}^A$  

**Gradients:**

$$
w_{ei,j}^h = \sum_A N_{,j}^A c_{ie}^A ; u_{ei,j}^h = \sum_A N_{,j}^A d_{ie}^A 
$$  
...
최종식:  
$$
\sum_e \underline{c}_e^T \underline{K}_e \underline{d}_e = \sum_e \underline{c}_e^T \underline{F}_e^{int} + \sum_{i=1}^{n_{sd}} \sum_{e \in E_N} \sum_A c_{ie}^A F_{ie}^{E_A}
$$  

! 헷갈리는 부분 정리!  

$c^A_e$ : 노드 A에서의 가상변위 벡터  
노드 인덱스: $A=1,2,..., n_{ne} \rightarrow$ 몇 개의 노드에 자유도가 있냐  
벡터 성분 인덱스: $ i=1,2,...,n_{sd} \rightarrow$  각 노드에서 성분이 몇개냐  

$$
C_e^A = \begin{bmatrix} C_1^A \\ C_2^A \\ C_3^A \end{bmatrix}_e
$$  

한 노드에서 $n_sd=3$ 의 성분을 가진 벡터라는 뜻이고,  

$$
C_e = \begin{Bmatrix} C_e^1 \\ C_e^2 \\ \vdots \\ C_e^{n_{ne}} \end{Bmatrix}
$$  

요소 전체에 대해서 모으면 한 노드당 3개씩이니까 전체는 $n_{ne} x n_{sd}$개 이다.  
$d$도 똑같이 하면 된다.  
...
\underline{K}_e = 
\begin{bmatrix}
\underline{k}_e^{11} & \underline{k}_e^{12} & \cdots & \underline{k}_e^{1nne} \\
\vdots & \ddots & & \vdots \\
\underline{k}_e^{nne1} & & \cdots & \underline{k}_e^{nnenne}
\end{bmatrix}
; \quad
\underline{F}_e^{int} = 
\begin{Bmatrix}
...
$$
\delta W_{int}^e = \int_{\Omega_e} \varepsilon(\mathbf{w})^T \mathbf{C} \varepsilon(\mathbf{u}) \,dV
$$  

여기서 $u$는 실제 변위, $w$는 가상변위  

노드 A의 내부 힘을 노드 B으 변위와의 관계로 쓰면  

$$
\mathbf{F}_e^{\text{int},A} = \sum_{B=1}^{n_{ne}} k_e^{AB} \mathbf{d}_e^B
$$  

전체를 한번에 쓰면  
$$
\begin{Bmatrix} \mathbf{F}_e^{\text{int},1} \\ \vdots \\ \mathbf{F}_e^{\text{int},n_{ne}} \end{Bmatrix} = \begin{bmatrix} k_e^{11} & \cdots & k_e^{1n_{ne}} \\ \vdots & \ddots & \vdots \\ k_e^{n_{ne}1} & \cdots & k_e^{n_{ne}n_{ne}} \end{bmatrix} \begin{Bmatrix} \mathbf{d}_e^1 \\ \vdots \\ \mathbf{d}_e^{n_{ne}} \end{Bmatrix}
$$  

$K$행렬 안에 있는 작은 $k$행렬들은 $n_{sd} x n_{sd}$ 크기를 가지고 있다. 행 인덱스 $A$는 어느 노드의 힘을 계산하는지를 나타내고 열 인덱스 $B$는 어느 노드의 변위를 곱해주냐는 뜻이다. 노드 B의 변위가 노드 A의 힘에 미치는 영향을 나타낸다고 이해하면 된다.  

그리고 작은 행렬들이 $n_{ne} x n_{ne}$가 모여서 전체 $K$행렬의 크기가 $(n_{ne} \times n_{sd})^2$가 된다.  

**The global matrix-vector equation**  

이제 최종식에서 디리클레 경계조건을 적용해주어야 한다.  
dof는 자유로운 노드의 갯수인데 디리클레 경계조건이 적용되면 값이 정해져서 우변으로 빼줘야 한다.  

따라서 $c,d$ 벡터의 크기는  
$$
\underline{d} : n_{ne} \times n_{sd} \\  
\underline{c} : n_{ne} \times n_{sd} - N_D
$$  

Q. c와 d 모두 자유도의 갯수인데 디리클레 경계조건이 적용돼서 값이 고정되면 두개 모두에서 $N_D$를 빼줘야 하는거 아닌가?  
A. c는 디리클레 경계조건이 적용된 노드에서는 값이 0으로 강제된다. 그래서 디리클레 노드는 애초에 dof가 없다고 본다. d에서도 물론 디리클레 경계조건이 적용된 노드가 있다. 하지만 d는 디리클레 값이 주어질 수 있으므로 $N_D$를 빼지 않는 것이다.  

이제 $c$ 벡터에 대한 크기가 바꼈기 때문에 행렬 $K$의 크기도 바뀐다.  

**global dof에 대해서 assembly**  

$$
\sum_e \underline{c}_e^T \underline{K}_e \underline{d}_e = \underline{c}^T \bar{K} \underline{d}  
(n_{ne} \times n_{sd} - N_D) \times (n_{ne} \times n_{sd})
$$  
