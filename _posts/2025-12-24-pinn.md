...
u_x_analy = -(nu * ext_traction / e_modul) * x_coord
u_y_analy = (ext_traction / e_modul) * y_coord

sigma_xx_analy = np.zeros_like(x_coord)
sigma_yy_analy = np.full_like(y_coord, ext_traction)
sigma_xy_analy = np.zeros_like(x_coord)

error_u = np.abs(u_pred - u_x_analy)
error_v = np.abs(v_pred - u_y_analy)
combined_error_disp = (error_u, error_v, np.zeros_like(error_u))
```
