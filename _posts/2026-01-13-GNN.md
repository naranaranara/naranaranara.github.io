...
- Laerner.py 에서 fused=True 조건 삭제
 ...
 1\. .feb: FEBio에 "이 문제를 이렇게 풀어줘"라고 주는 입력 파일 

    - 메시(노드/요소), 재료(E,nu 등), 경계조건, 하중, 접촉 정의, 출력 정의  
    - 입력값  
    - .feb 만드는 법  
      - FEBio에서 모델을 만들고 export  
      - 파이썬으로 템플릿 .feb를 복제해서 파라미터(압력/재료/형상)만 바꿔가며 자동 생성  
      
 2\. .csv: FEBio가 계산해서 뽑아주는 결과/로그 데이터  

    - .csv는 출력 설정에 따라 자동으로 만들어짐  
    - feb 파일 안에 csv 출력 설정이 있음  
    
 3\. .npz/.json/.pt: 학습에 쓰기 좋게 정리/포장 한 데이터셋 파일들  

    - .npz(NumPy 압축파일)  
      - 큰 배열 덩어리를 저장하는 용도  
      - node_coords, displacement, stress, time 등  

    - .json (메타데이터/설정/인덱스파일)  
      - 설명서/목차 같은 정보  
      - 시뮬레이션 케이스 리스트, 노드/요소 개수, 파트 id, boundary tag 맵  

    - .pt (PyTorch 저장 파일)  
      - 학습에 바로 쓰려고 torch tensor나 PyTorch Geometric의 Data 객체로 저장해둔 것  
      - x(노드 피쳐: 좌표, 재료, bc tag, load 값등), edge_index, edge_attr, y  
